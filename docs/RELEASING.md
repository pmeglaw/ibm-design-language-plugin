...
Keep frozen release ZIPs and their manifests unchanged. Make new work on a task branch. Change the source plugin and increment its version only for an intentional release; the validator deliberately rejects drift from the current locally prepared snapshot (1.1.12) until a new release is prepared.
